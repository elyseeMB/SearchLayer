...
CREATE DATABASE search_layer_database;
GRANT ALL PRIVILEGES ON DATABASE search_layer_database TO search;
EOF

psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d search_layer_database <<-EOF
